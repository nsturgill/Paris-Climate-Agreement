...
preddat = subset(combo[,c(7:24)])               # missing data and impute  
preddat$damage = as.numeric(damage$`% Damage`)
...
regdat = data.frame(avgdat)                     # move data to new data frame and convert CapMt
...
selectdat = data.frame(matrix(ncol = 14, nrow = 187)) 
...
for(i in 1:length(amod1$`Sum Sq`)){             # if SSTrt > .1, keep 
  if(amod1$`Sum Sq`[i] > .1000){                # this allows for greater variability in model
    selectdat[,count] = subdat[,i]
    coldat[count] = nam[i]
    count= count+1
  }
}
selectdat = selectdat[,-c(8:13)]
